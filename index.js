                ...
                if (isBanned) return reply(aml.baned)
{
                ...
                try {
                    ...
                    .then((res) => {
                        ...
                        axios.get(`https://tinyurl.com/api-create.php?url=${dl_link}`)
                        .then((a) => {
                            if (Number(filesize) >= 4000) return sendFileFromUrl(from, thumb, `┌───〔 *_𝙹𝙰𝚁𝚅𝙸𝚂_* 〕
┊
└───────────𔒝
┌───〔ᴠɪᴅᴇᴏ〕
┊\`\`\`🤖ᴛɪᴛʟᴇ : ${title}\`\`\`
                ...
                if (isBanned) return reply(aml.baned)
             {
                ...
                try {
                    ...
                    .then((res) => {
                        ...
                        axios.get(`https://tinyurl.com/api-create.php?url=${dl_link}`)
                        .then((a) => {
                            if (Number(filesize) >= 3000) return sendFileFromUrl(from, thumb, `┌───〔 *_𝙹𝙰𝚁𝚅𝙸𝚂_* 〕
┊
└───────────𔒝
┌───〔sᴏɴɢ〕
┊\`\`\`🤖ᴛɪᴛʟᴇ : ${title}\`\`\`
...
/*========================================================*/
	...
	Ruri.sendMessage(from, `Prefix: ${prefix}`, text, { sendEphemeral: true, thumbnail: fs.readFileSync('./lib/logo.jpeg', 'base64')}) 			
    break
}
